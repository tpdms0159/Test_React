...
function Home() {
  ...
  return(
    <div>
      <h1>{loading ? "nothings" : `Today's Hot Movie is ${movies[0].movieNm}`}</h1>
      {loading ? <h1>Movie loading...</h1> : 
      <div>
          {movies.map((movie) => { 
            return <Movie key={movie.rank} date={movie.openDt}id={movie.rank} rank={movie.rank} name={movie.movieNm }/>})}
        </div>}
    </div>
  )
